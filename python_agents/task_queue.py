...
class TaskQueue:
    ...
    def __init__(
        self,
        max_workers: int = int(os.getenv("QUENBOT_TASK_QUEUE_WORKERS", str(max(2, min(6, (os.cpu_count() or 4) // 3 + 1))))),
    ...
    async def wait_for_result(self, task_id: str, timeout: Optional[float] = None) -> Optional[dict]:
        """Wait for a submitted task to complete and return its result envelope."""
